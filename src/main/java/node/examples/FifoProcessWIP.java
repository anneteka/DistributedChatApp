...
//		System.out.println("Adding process " + id + " to list: " + input);
    ...
    public static void scanConfigFile(int id) {
        String configFile = Paths.get("src", "main", "resources", "config_file.txt").toString();
...
//				/*
    ...
    public static void sendMessage(Message m) {
        try {
            String[] destinationInfo = m.getMetaData().getProcess();
...
//				data.setWriter(new ObjectOutputStream(data.getSocket().getOutputStream()));
        ...
        (new Thread(() -> {
            ...
            try {
                ...
                while (true) {
                    ...
                    (new Thread(() -> receiveMessages(s))).start();
                }
            } catch (IOException e) {
                e.printStackTrace();
            }
        })).start();
        System.out.println("started server "+ serverName+" "+port);
    }
    ...
    public static void receiveMessages(final Socket s) {
        System.out.println("server started receiving messages");
        try {
            ...
            while ((msg = (Message)in.readObject()) != null) {
                ...
                (new Thread(() -> unicastReceive(m, s))).start();
            }
        } catch (IOException e) {
            e.printStackTrace();
    ...
    public static void unicastReceive(Message m, Socket s) {
        ...
        int id = Integer.parseInt(data.getProcess()[0]);
...
//				System.out.println("Queue: v_time = " + v_time + "; msgTime = " + msg.getTimestamp());
    ...
    public static void main(String[] args) throws IOException {
        ...
        String[] info = list.get(id).getProcess();
        if (info == null)
            return;
